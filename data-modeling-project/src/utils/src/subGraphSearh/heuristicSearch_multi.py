...
def heuristic_search(graph, target_subgraph, num_results, graph_node_lon='x', graph_node_lat='y', search_step=0.001,
                     search_strategy={'random': 0.2, 'top_adj': 0.4}, max_iter=500, error = 0.00001):
    """
    随机选取初始节点并生成子图
    计算相似度
    ...
    for i in range(max_iter):
        ...
        if tmp_error < error:
            ...
            iTerminate = 0
        results = [subgraph for _,__,subgraph in sorted(candidate_subgraphs, key=lambda x: -x[0])[:num_results]]
        graphs_id = 0
        list_central_node = []
        for heuristic_search_graph in results:
            list_central_node.append(get_graph_central_node(heuristic_search_graph))
        df_central_nodes = pd.DataFrame()
        df_central_nodes['node'] = list_central_node
        # df_central_nodes.to_csv('./demo/result_top_central_node/'+str(i)+'.csv',index=False)
        df_central_nodes.to_csv('./demo/jianshe/'+str(i)+'.csv',index=False)
             
        # graphs_id = 0
        # for heuristic_search_graph in result:
        #     ox.save_graph_geopackage(heuristic_search_graph, "./zhenlong/similarity_sub_g_"+str(graphs_id)+".gpkg")
        #     graphs_id +=1           
        # if i % 100 ==0:
        #     df_central_nodes = pd.DataFrame()
        #     df_central_nodes['node'] = list(central_nodes)
        #     df_central_nodes.to_csv('./demo/result_node/'+str(i)+'.csv',index=False)

            #print(f"There are no dominating results for the last iterations, iteration terminate at iter: {i}.")
            # result = [subgraph for _,__,subgraph in sorted(candidate_subgraphs, key=lambda x: -x[0])[:num_results]]
            # list_sim = []
            # for heuristic_search_graph in result:
            #     list_sim.append(cal_total_weighted_similarity(target_subgraph, heuristic_search_graph))  
            # print(i,cal_total_weighted_similarity(target_subgraph, result[0]),sum(list_sim)/len(list_sim))          
    ...
    # return [subgraph for _,__,subgraph in sorted(candidate_subgraphs, key=lambda x: -x[0])[:num_results]] # bug_fix : for _,subgraph 改为 for _,__,subgraph
        ...
        for heuristic_search_graph in result:
            list_sim.append(cal_total_weighted_similarity(target_subgraph, heuristic_search_graph))  
        print(i,  round( cal_total_weighted_similarity(target_subgraph, result[0]),3 ) ,round( sum(list_sim)/len(list_sim) , 3) , len(list(central_nodes)))   