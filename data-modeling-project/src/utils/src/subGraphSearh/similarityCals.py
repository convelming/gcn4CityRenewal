...
def cal_cluster_coe_diff(graph_1, graph_2):
    ...
    C_graph1 = nx.average_clustering(nx.Graph(graph_1))
    C_graph2 = nx.average_clustering(nx.Graph(graph_2))
...
def cal_total_weighted_similarity(graph_1, graph_2, weights={'kl': 0.1, 'sp': 0.3, 'cluster': 0.4, 'node': 0.1, 'edge': 0.1},
    ...
    similarity = (weights['kl'] *(1-kl_div) +
                  ...
                  weights['node'] * (node_similarity) +weights['edge'] * (edge_similarity))
    #
    if show:
        print(
