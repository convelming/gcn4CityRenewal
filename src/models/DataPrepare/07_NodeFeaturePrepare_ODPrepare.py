...
base_data_folder = './src/models/base_data/'
middle_data_floder = './src/models/middle_data/'
area_id = 'osmid'
df_od = pd.read_csv(base_data_folder+'202305广州通勤数据V2/广州通勤表_202305_rndCoord.csv')
...
gpd_source_grid.to_file(middle_data_floder+'od_grid.shp', driver='ESRI Shapefile', encoding='utf-8')
gpd_source_grid = []

gpd_grid = gpd.read_file(middle_data_floder+'od_grid.shp')
...
df_od_info = pd.read_csv(base_data_folder+'202305广州通勤数据V2/广州通勤明细表V2_202305.txt')
...
df_od.to_csv(middle_data_floder+'base_od.csv',index=False)
